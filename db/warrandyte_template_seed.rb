...
class DestructiveTemplateLoad
  def clear_out_all_data_and_load_warrandyte_template_plan
    ...
    tag_with[:not_alone] << Task.find_or_create_by_custom_and_name_and_event_id(false, "Define responsibilities and make arrangements for all family to be transported safely.", event.id)#!alone
    ...
    t = Task.find_or_create_by_custom_and_name_and_event_id(false, "Take the car, pets and your Emergency Kit and head for your relative's or friend's place. Remain there until the danger has passed.", event.id)#alone and pets
    tag_with[:alone] << t
    tag_with[:pets] << t

    t = Task.find_or_create_by_custom_and_name_and_event_id(false, "Take the cars, all family members, pets and our Emergency Kit and head for our relative's or friend's place. Remain there until the danger has passed.", event.id)#!alone and pets
    tag_with[:not_alone] << t
    tag_with[:pets] << t

    t = Task.find_or_create_by_custom_and_name_and_event_id(false, "Take the cars, all family members and our Emergency Kit and head for our relative's or friend's place. Remain there until the danger has passed.", event.id)#!alone !pets
    tag_with[:not_alone] << t
    tag_with[:not_pets] << t
    ...
    tag_with[:pets] << Task.find_or_create_by_custom_and_name_and_event_id(false, "Make sure pets are restrained.", event.id)#pets
