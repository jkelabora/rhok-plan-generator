...
Task.find_or_create_by_custom_and_name_and_event_id(false, "Pack emergency kit in a large plastic container for quick transportation- photographs, backup discus, medications and prescriptions, insurance documents, passports, cameras, paintings, jewellery, etc. (see Emergency Kit below).", event.id)
...
Task.find_or_create_by_custom_and_name_and_event_id(false, "Divert the home phone to- Tom’s mobile.", event.id)
...
Task.find_or_create_by_custom_and_name_and_event_id(false, "Turn off Gas suuply at main", event.id)
...
Task.find_or_create_by_custom_and_name_and_event_id(false, "Monitor 774 KHz. AM and www.cfa.vic.gov.au/incidents/incident_summary.htm on computer (assuming power is still available).", event.id)
...
Task.find_or_create_by_custom_and_name_and_event_id(false, "Jewellry", event.id)
