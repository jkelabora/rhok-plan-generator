...
Task.find_or_create_by_custom_and_name_and_event_id(false,pets, "Make arrangements for all animals to be transported safely.", event.id)
Task.find_or_create_by_custom_and_name_and_event_id(false,!alone, "Define responsibilities and make arrangements for all family to be transported safely.", event.id)
Task.find_or_create_by_custom_and_name_and_event_id(false,!alone, "Practise our leave early plan with all the family. Make sure everyone knows their role and how scary a bushfire can be.", event.id)
Task.find_or_create_by_custom_and_name_and_event_id(false,alone, "Practise how to leave early.", event.id)
...
Task.find_or_create_by_custom_and_name_and_event_id(false,children, "Advise the children's schools of our plans to leave early on a SEVERE, or greater, fire risk day.", event.id)
...
Task.find_or_create_by_custom_and_name_and_event_id(false,!alone, "Discuss with the entire family where we would shelter in the house if unable to leave.", event.id)
Task.find_or_create_by_custom_and_name_and_event_id(false,alone, "Identify where to shelter in the house if unable to leave.", event.id)
Task.find_or_create_by_custom_and_name_and_event_id(false,children, "Inform the children that we will expect them to stay at school should a fire start during school hours, as both schools have fire protocols in place for emergencies.", event.id)
...
Task.find_or_create_by_custom_and_name_and_event_id(false,alone and pets, "Take the car, pets and your Emergency Kit and head for your relative's or friend's place. Remain there until the danger has passed.", event.id)
Task.find_or_create_by_custom_and_name_and_event_id(false,!alone and pets, "Take the cars, all family members, pets and our Emergency Kit and head for our relative's or friend's place. Remain there until the danger has passed.", event.id)
Task.find_or_create_by_custom_and_name_and_event_id(false,!alone !pets, "Take cars, all family members and our Emergency Kit and head for our relative's or friend's place. Remain there until the danger has passed.", event.id)
...
Task.find_or_create_by_custom_and_name_and_event_id(false,children, "If a school day, notify the teachers our children will not be attending until further notices (see Emergency Contact Numbers)", event.id)
...
Task.find_or_create_by_custom_and_name_and_event_id(false, "Check the Warrandyte Fire Watch site warrandyte.org.au/fire-watch/", event.id)
Task.find_or_create_by_custom_and_name_and_event_id(false,pets, "Place the pets in the downstairs laundry area with water and food.", event.id)
...
Task.find_or_create_by_custom_and_name_and_event_id(false,!alone, "Remove from Emergency Kit, and everyone put on their fire protection clothing and gear - including masks. Hand out torches and fill and place metal buckets at front and back doors, with mops and wheelie bins filled with water.", event.id)
Task.find_or_create_by_custom_and_name_and_event_id(false,alone, "Remove from Emergency Kit, and put on fire protection clothing and gear - including mask. Grab the torch and fill and place metal buckets at front and back doors, with mops and wheelie bins filled with water.", event.id)
...
Task.find_or_create_by_custom_and_name_and_event_id(false,pets, "Make sure pets are restrained.", event.id)
...
Task.find_or_create_by_custom_and_name_and_event_id(false,pets, "Try to manage the pets but do not let anyone go after them should they escape - stay together.", event.id)
...
Task.find_or_create_by_custom_and_name_and_event_id(false,pets, "Dog leads and food, bird seed and water", event.id)
